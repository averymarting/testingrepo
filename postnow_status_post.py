...
# Identity of the repo/runner executing this job right now — used only for
# the soft cross-repo lock (LOCKED_BY / LOCKED_AT columns), so two different
# repos pointed at the same Sheet1 row don't post for the same account at
# the same time.
...
#      (currently just ACCOUNT_ROW), read via get_*_env()
...
# ═══════════════════════════════════════════════════════════════════════════
#
#  ACCOUNT_ROW is the ONE setting that intentionally stays outside the sheet:
#  it's "which row does this repo/runner belong to", set once via the
#  workflow_dispatch input or a repo variable, and left alone until you
#  change it on purpose. Every other setting now lives in Sheet1 and is
#  re-read fresh on every posting cycle — see load_account_config().
...
# Master sheet: Sheet1 = credentials + live settings, Report = daily stats + top posts
...
# ═══════════════════════════════════════════════════════════════════════════
#
#  Expected Sheet1 header (case-insensitive), columns A onward:
#
...
#  LOCKED_BY | LOCKED_AT   (last two are optional — only needed if you want
#                            the cross-repo "don't double-post" lock)
#
#  Any column can be left blank for a row — a sensible default is used.
...
def load_account_config(force_refresh=False):
    ...
    img_ratio_raw = _parse_pct(col("IMAGE_RATIO"), 0.60)
    vid_ratio_raw = _parse_pct(col("VIDEO_RATIO"), 0.40)
    ...
    cfg = {
        ...
        "hashtags_enabled_image":   _parse_bool(col("HASHTAGS_ENABLED_IMAGE"), True),
        "hashtags_enabled_video":   _parse_bool(col("HASHTAGS_ENABLED_VIDEO"), False),
        "link_enabled_image":       _parse_bool(col("LINK_ENABLED_IMAGE"), True),
        "link_enabled_video":       _parse_bool(col("LINK_ENABLED_VIDEO"), True),
        "link_percentage":          _parse_pct(col("LINK_PERCENTAGE"), 1.0),
        "max_image_bytes":          int(_parse_plain_float(col("MAX_IMAGE_MB"), 2.0) * 1024 * 1024),
        "enable_report":            _parse_bool(col("ENABLE_REPORT"), False),
        "top_posts_count":          _parse_int(col("TOP_POSTS_COUNT"), 5),
        "top_posts_within":         _parse_int(col("TOP_POSTS_WITHIN"), 30),
        "post_plan_sheet_name":     col("POST_PLAN_SHEET_NAME") or "Sheet1",
...
def print_config_summary():
    cfg = _cfg()
    print("── Run config (live from Google Sheet, re-checked every cycle) ──")
...
def run_once():
    ...
    # right away, and check/refresh the cross-repo lock at the same time.
    cfg = refresh_account_config()
